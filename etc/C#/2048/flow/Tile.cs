...
namespace flow
{
	public class Tile : Vector
	{
		...
		public void Draw() 
		{
			...
			char Ch ;
			if (Value > 7) { CC = ConsoleColor.Red; }
			if (Value > 31) { CC = ConsoleColor.DarkRed; }
			if (Value > 127) { CC = ConsoleColor.DarkYellow; }
			if (Value > 500) { CC = ConsoleColor.Yellow; }
			Ch = ((int)Math.Log(this.Value,2)).ToString()[0];
			//Ch = (float)this.Value/Main.Instance.BestValue;
