//fork of https://github.com/rburns/ansi-to-html
'use strict';
...
function generateOutput(stack, token, data, options) {
    ...
    } else if (token === 'xterm256bg') {
        result = pushBackgroundColor(stack, options.colors[data]);
    } else if (token === 'rgb') {
        result = pushForegroundColor(stack, `rgb(${data.r},${data.g},${data.b})`);
    } else if (token === 'rgbbg') {
        result = pushBackgroundColor(stack, `rgb(${data.r},${data.g},${data.b})`);
    }
...
function tokenize(text, options, callback) {
    ...
    function removeRGB(m,r,g,b) {
        callback('rgb',{r,g,b});
        return '';
    }

    function removeXterm256Background(m, g1) {
        callback('xterm256bg', g1);
        return '';
    }

    function removeRGBBackground(m,r,g,b) {
        callback('rgbbg',{r,g,b});
        return '';
    }
    ...
    var tokens = [{
        ...
        pattern: /^\x1b\[38;2;(\d+);(\d+);(\d+)m/,
        sub: removeRGB
    }, {
        pattern: /^\x1b\[48;5;(\d+)m/,
        sub: removeXterm256Background
    }, {
        pattern: /^\x1b\[48;2;(\d+);(\d+);(\d+)m/,
        sub: removeRGBBackground
    }, {
        pattern: /^\n/,
        sub: newline
    }, {
