...
Compiler.prototype.math = function(data) {
  var temp1 = '%'+(this.stackAddress-0x2);var temp2 = '%'+(this.stackAddress-0x3);
  ...
  if(data.length == 3){
    ...
    if(data[1] == '['){
      if(data[3] == ']'){
        ...
        var f = operators['_arrayget'].format(this.compileVariable(data[0]),this.compileVariable(data[2]),temp1);
...
operators['_arrayget'] = "mov %this+7 {0} \nadd %this+4 {1} \nmov {2} %0xcc";//(array,index,out value)
operators['_arrayset'] = "mov %this+8 {0} \nadd %this+5 {1}";// \nmov %0xcc {2}";//(array,index,value)
...
keywordDictionary['var'] = function(data)
{
  ...
  if(data[1] == 'array'){
    if(data[2] == '(' && data[4] == ')'){
      ...
      this.vars[v] = {v:'$0x'+a.toString(16),type:'array'}
