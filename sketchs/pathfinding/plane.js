function Wplane(n) {
  ...
  this.checkForWall = function(x,y,dir)
  { 
    return this.tiles[x][y].walls[dir].b;
  }
...
Tile.prototype.draw = function(b,colors){ 
  push();
  rectMode(RADIUS);
  stroke(0,100);
...
Wall.prototype.draw = function(colors){ 
  push();
...
function Turtle(p,ID,bDay) {
  ...
  this.advance = function()//this.nextTurn
  {
    if(this.queue.length > 0){this.moveToTile(this.queue.shift());//this.queue.pop()
    }else{this.onPathEnd();}
  }
  
  this.onPathEnd = function() 
  {
    if(true){this.buildWall();}//todo:check if we reached our goal
    //randomSeed(random(0,500));
    this.pathTo(floor(random(0,this.plane.tiles.length)),floor(random(0,this.plane.tiles.length)));
  }
  
  this.pathTo = function(x,y)
  {
    this.queue = pathfinder.findCords(this.x,this.y,x,y);
  }
  
  this.buildWall = function()
  {
    this.plane.tiles[this.x][this.y].walls[this.dir].b = !this.plane.tiles[this.x][this.y].walls[this.dir].b;
    this.plane.pathfinder.isValid = false;
    try{ 
  ...
  this.moveToTile = function(tile)
  {
    if(this.x == tile.x&&this.y > tile.y){
      this.move(0);
    }else if(this.x < tile.x&&this.y == tile.y){
      this.move(1);
    }else if(this.x == tile.x&&this.y < tile.y){
      this.move(2);
    }else if(this.x > tile.x&&this.y == tile.y){
