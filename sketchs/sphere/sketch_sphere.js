...
function draw(){
  ...
  for (var i = 0; i < points.length; i++) {
    var point1 = points[i];
    var x = point1.getPos().x;var y = point1.getPos().y;
...
Point.prototype.getPos = function() {
  return createVector(this.sx,this.sy);
}
...
Point.prototype.rotate = function(d) {
  d = createVector(d.x,d.y);
  var v = rotator.applyMatrix(rotator.quaternionToMatrix(rotator.makeQuaternion(createVector(-d.y,d.x,0).normalize(),(d.mag()*0.1)%360)),clone(this));
...
/*  this.yrot += y;
  ...
  this.xrot = atan2(v.y,v.x);
  this.xrot = modulo(this.xrot,TWO_PI);
  this.yrot = atan2(v.z,v.x);
  if(abs(this.yrot) > HALF_PI){
    this.yrot = (this.yrot > 0)?(PI - this.yrot):-(TWO_PI - (PI - this.yrot));
    //this.xrot += PI;
    //step = -step;
  }
  this.sx = modulo(this.xrot,TWO_PI)/HALF_PI;
  this.sy = -sin(this.yrot);
  return this;//??
}
...
function input(dir){
  var rom = [{x:0,y:1},{x:1,y:0},{x:0,y:-1},{x:-1,y:0}];
  var rom2 = [{x:1,y:1},{x:-1,y:1},{x:-1,y:-1},{x:1,y:-1}];
  centerPoint.rotate(rom[dir]);
  for (var i = 0; i < rom2.length; i++) {
    points[i] = clone(centerPoint).rotate(rom2[i]);
  }
